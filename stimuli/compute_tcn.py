...
if __name__ == '__main__':
    ...
    input_imagewidth = 24
    input_imageheight = 24

    layer_channels = [3, 48, 48, 96]
    n_classes = 48
    ...
    layer_pooling_enable = [True, True, True, False, False]
    ...
    for i in range(num_layers):
        #=======================================================FIXED===============================================
        weightmemorywrites += int(np.ceil(layer_ni[i] / (ni / weight_stagger)) * layer_k * layer_k * rounded_no[i])
        threshold_writes += layer_no[i]
        memwrites.append({'layer': i, 'ni': layer_ni[i], 'no': rounded_no[i], 'weight_writes': weightmemorywrites,
